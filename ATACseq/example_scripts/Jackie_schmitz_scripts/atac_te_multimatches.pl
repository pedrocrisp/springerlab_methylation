#! /usr/bin/perl -w
# take output of bedtools closest (te and atac data) and determine most overlap for multi te matches
# atac_te_file.pl
# 18_Jan_2017
# Jaclyn_Noshay
...
my %multi;
my ($chr, $start, $end, $te_chr, $te_start, $te_end, $te_id, $dist, $class, $overlap, $name);
while (my $line = <$in_fh>) {
  chomp $line;
  my ($chr, $start, $end, undef, undef, undef, $te_start, $te_end, undef, undef, undef, $te_id, $dist, $class, $overlap) = split ("\t", $line);
...
my ($chr2, $start2, $end2, $te_chr2, $te_start2, $te_end2, $te_id2, $dist2, $class2, $overlap2, $name2);
while (my $line2 = <$in2_fh>) {
  chomp $line2;
  ($chr2, $start2, $end2, $te_chr2, undef, undef, $te_start2, $te_end2, undef, undef, undef, $te_id2, $dist2, $class2, $overlap2) = split ("\t", $line2);

  $name2 = $chr2 . "_" . $start2 . "_" . $end2;

  #print "$multi{$name2}{$overlap2}\t";
