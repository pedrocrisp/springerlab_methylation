...
print $out_fh "chr\tstart\tstop\twidth\tstrand\tclass\tte_start\tte_end\toverlap\tratio_overlap\tte_class\tte_family\tte_id\tatac_te_dist\n";

my %peaks;
my $header = <$in_fh>;
while (my $line = <$in_fh>) {
  chomp $line;
  my ($chr, $start, $end, $width, $strand) = split ("\t", $line);
...
while (my $line2 = <$in2_fh>) {
  chomp $line2;
  my ($chr, $start, $end, $width, undef, undef, undef, undef, $te_start, $te_end, undef, undef, undef, $te_id, $dist, $class, $overlap) = split ("\t", $line2);

  # 1	100233092	100234204	1113	*	1	rtracklayer	sequence_feature	100232826	100240953	.	*	.	ID=RLG00018B73v400579;sup=RLG	0	partial_overlap	7015
