...
print $out_fh "chr\tstart\tstop\twidth\tstrand\tclass\tte_start\tte_end\toverlap\tratio_overlap\tte_class\tte_family\tte_id\tatac_te_dist\tte\tstart\tend\tstrand\tigv_coords\tdisjoined_length\tnested\tin_intron\tltr_coord\torder\n";
...
while (my $line2 = <$in2_fh>) {
  chomp $line2;
  my ($chr, $start, $end, $width, $strand, $class, $te_start, $te_end, $overlap, $ratio_overlap, $te_class, $te_family, $te_id, $atac_dist) = split ("\t", $line2);

  # chr	start	stop	width	strand	class	overlap	te_class	te_family	te_id	atac_te_dist
