...
# Peter Crisp
# 2017-04-08
# R script to filter CG/CHG?CHH tile files based on C/CT and G/GA sites and coverage filters
...
minCHH_A_percent <- args[11]
minCHH_GA_cov <- args[12]
minCHG_A_percent <- args[13]
minCHG_GA_cov <- args[14]
minCG_A_percent <- args[15]
minCG_GA_cov <- args[16]
...
mC_tile_table %>%
  ...
  mutate(cov= CT/chh_sites, GA_cov = GA/chh_sites, A_percent = (GA-G)/GA) %>%
  when(tile_list == FALSE ~ ., ~ inner_join(tile_list, "tile")) %>%
  filter(chh_sites >= as.double(minCHHs) & cov >= as.double(minCHH_cov) & GA_cov >= as.double(minCHH_GA_cov) & A_percent < as.double(minCHH_A_percent)) %>%
...
mC_tile_table %>%
  unite(tile, chr, start,end, sep=":", remove = F) %>%
  mutate(cov= CT/cg_sites, GA_cov = GA/cg_sites, A_percent = (GA-G)/GA) %>%
  when(tile_list == FALSE ~ ., ~ inner_join(tile_list, "tile")) %>%
  filter(cg_sites >= as.double(minCGs) & cov >= as.double(minCG_cov) & GA_cov >= as.double(minCG_GA_cov) & A_percent < as.double(minCG_A_percent)) %>%
...
mC_tile_table %>%
  unite(tile, chr, start,end, sep=":", remove = F) %>%
  mutate(cov= CT/chg_sites, GA_cov = GA/chg_sites, A_percent = (GA-G)/GA) %>%
  when(tile_list == FALSE ~ ., ~ inner_join(tile_list, "tile")) %>%
  filter(chg_sites >= as.double(minCHGs) & cov >= as.double(minCHG_cov) & GA_cov >= as.double(minCHG_GA_cov) & A_percent < as.double(minCHG_A_percent)) %>%
