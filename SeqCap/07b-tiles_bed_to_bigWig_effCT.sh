...
########## Run #################
        ...
        perl ~/gitrepos/springerlab_methylation/SeqCap/met_context_window_effectiveCT.pl ./BSMAPratio/${ID}_BSMAP_out.txt 100

        #move output into tiles folder
        mv -v ./BSMAPratio/${ID}_BSMAP_out.txt.100.*.bed ./tiles_effCT/
        ...
        --args ${ID} tiles_effCT ${reference_tile_file}
        ...
        bedGraphToBigWig "./tiles_effCT/${ID}_BSMAP_out.txt.100.CG.fixed.sorted.bg" ${chrom_sizes} \
        "./tiles/${ID}_BSMAP_out.txt.100.CG.bigWig"
        bedGraphToBigWig "./tiles_effCT/${ID}_BSMAP_out.txt.100.CHG.fixed.sorted.bg" ${chrom_sizes} \
        "./tiles/${ID}_BSMAP_out.txt.100.CHG.bigWig"
        bedGraphToBigWig "./tiles_effCT/${ID}_BSMAP_out.txt.100.CHH.fixed.sorted.bg" ${chrom_sizes} \
